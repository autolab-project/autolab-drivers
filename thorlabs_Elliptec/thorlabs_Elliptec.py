...
def import_Decimal():
    ...
    if not 'System' in sys.modules:
        import System ## has to be imported after clr
    else:
...
class TwoPosSlider():
    ...
    def __init__(self, ELLDev):
        self.Device = ELLDev
        self.Decimal = import_Decimal()

    def GetPosition(self):
        if self.Device.GetPosition():
            pos = self.Decimal.ToDouble(self.Device.Position)
        return(pos)
...
class RotationMount():
    ...
    def MoveAbsolute(self, pos):
        ...
        return(self.Device.MoveAbsolute(self.Decimal(target)))
    ...
    def MoveRelative(self, step=None):
        ...
        if step is None:
            step = self.JogStep
        return(self.Device.MoveRelative(self.Decimal(step)))
...
class LinearStage():
    ...
    def MoveAbsolute(self, pos):
        ...
        """
        target = pos + self.HomeOffset
        ret = self.Device.MoveAbsolute(self.Decimal(target))
        if not ret:
    ...
    def MoveRelative(self, step=None):
        ...
        if step is None:
            step = self.JogStep
        ret = self.Device.MoveRelative(self.Decimal(step))
        if not ret:
    ...
    def GetPos2(self):
        return self.pos2
    def GoTo1(self):
        self.Device.MoveAbsolute(self.Decimal(self.pos1))
    def GoTo2(self):
        self.Device.MoveAbsolute(self.Decimal(self.pos2))
